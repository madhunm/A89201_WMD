...
#define GDU_FAULT_CLEAR_ALL (0xFFFFu)

volatile uint16 u16Gdu_diffAmpGain = DIFF_GAIN_X10;

 /**
 * @brief initializes the GDU register settings
 */
...
void Gdu_Initialization()
{
	...
	/* Driver config: VT=0x18 (1.2V), VQT=0x10 (1.6us), debounce */
  u16TxData = (0x18u << DRIVER_VT_Pos) | (0x10u << DRIVER_VQT_Pos);
  GduMaster_Write(eGDUSLAVE_DRIVER_CONFIG, u16TxData);
	
	/* Diff amp gain */
  GduMaster_Write( eGDUSLAVE_DI_CSA, ( u16Gdu_diffAmpGain & 0x7u) );
	
	/* Enable LIN (20k baud) */
  u16TxData = ( CFG_LEN | CFG_LBB_20k );
  GduMaster_Write( eGDUSLAVE_CONFIG, u16TxData );
	
	/* Enable bridge */
  u16TxData = GATE_ENBR;
  GduMaster_Write( eGDUSLAVE_GD_CONTROL, u16TxData);
	
	/* Disable Q&A timeout */
	u16TxData = FMASK_WD | FMASK_ETO;
	GduMaster_Write( eGDUSLAVE_MASK, u16TxData );
}

uint16 Gdu_GetStatus()
{
	return GduMaster_Read(eGDUSLAVE_STATUS);
}

void Gdu_ClearGduStatus()
{
	/*Clear Status Register */
  GduMaster_Write(eGDUSLAVE_STATUS,GDU_FAULT_CLEAR_ALL);
}

void Gdu_SetDiffAmpGain()
{
	GduMaster_Write(eGDUSLAVE_DI_CSA, u16Gdu_diffAmpGain & 0x7 );
}
